...
if (isset($_GET['imprimir']) && isset($_GET['user_id'])) {
    ...
    $stmt = db()->prepare("SELECT u.*, r.photo, r.profession, r.formation
                           FROM users u
...
/* Carteira: 90mm × 135mm (proporção 2:3) — exibida em 2× na tela */
.card-outer {
...
.card-field label {
    ...
    text-transform: uppercase; letter-spacing: .8px; color: #888; margin-bottom: 1px;
}
...
<div class="print-bar">
    ...
    <a href="<?= BASE_URL ?>/admin/carteira.php?user_id=<?= $uid ?>">← Voltar</a>
...
<div class="page-wrap">
    <div class="card-outer">
        ...
        <div class="card-footer">
            ...
            <?php if ($validade): ?>
            <div class="validade">
                Válida até<strong><?= e($validade) ?></strong>
            </div>
            <?php endif; ?>
        </div>
    </div>
...
$users     = db()->query("SELECT id, full_name, matricula_apejese, adimplente FROM users ORDER BY full_name ASC")->fetchAll();
$validade  = getSetting('carteira_validade', '');
    ...
    <div class="col-lg-8">
        ...
        <div class="card admin-card mb-4">
            ...
            <div class="card-body text-center py-4">
                <?php
                $photoUrl  = $selectedUser['photo'] ? UPLOAD_URL . $selectedUser['photo'] : null;
                ...
                $corSit    = ($selectedUser['adimplente'] ?? 1) ? '#28a745' : '#dc3545';
                ?>
                <div style="display:inline-block;width:200px;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.2);text-align:left;font-family:Arial,sans-serif;">
                    <div style="background:linear-gradient(135deg,#1b3a6b,#0f2347);padding:10px 12px;display:flex;align-items:center;gap:8px;">
                        <div style="color:#fff;line-height:1.2;">
                            <div style="font-size:8pt;font-weight:800;letter-spacing:1px;">APEJESE</div>
                            <div style="font-size:5.5pt;opacity:.7;text-transform:uppercase;">Carteira de Associado</div>
                        </div>
                    </div>
                    ...
                    <div style="background:#fff;padding:10px 12px;">
                        <div style="display:flex;gap:8px;margin-bottom:8px;">
                            <div style="width:42px;height:52px;border-radius:5px;overflow:hidden;border:2px solid #1b3a6b;background:#dde3ee;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                <?php if ($photoUrl): ?>
                            ...
                            <div style="flex:1;min-width:0;">
                                <div style="font-size:7pt;font-weight:800;color:#1b3a6b;line-height:1.2;margin-bottom:2px;"><?= e($selectedUser['full_name']) ?></div>
                                <?php if (!empty($selectedUser['profession'])): ?>
                                <div style="font-size:5.5pt;color:#555;"><?= e($selectedUser['profession']) ?></div>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div style="height:1px;background:linear-gradient(90deg,transparent,#c9a227,transparent);margin-bottom:7px;"></div>
                        <div style="font-size:5.5pt;color:#888;text-transform:uppercase;font-weight:700;margin-bottom:1px;">Matrícula</div>
                        <div style="font-size:7pt;font-weight:600;color:#222;margin-bottom:5px;"><?= $selectedUser['matricula_apejese'] ? e($selectedUser['matricula_apejese']) : '—' ?></div>
                        <div style="font-size:5.5pt;color:#888;text-transform:uppercase;font-weight:700;margin-bottom:1px;">CPF</div>
                        <div style="font-size:7pt;font-weight:600;color:#222;margin-bottom:5px;"><?= $selectedUser['cpf'] ? e($selectedUser['cpf']) : '—' ?></div>
                        <div style="font-size:5.5pt;color:#888;text-transform:uppercase;font-weight:700;margin-bottom:1px;">Nascimento</div>
                        <div style="font-size:7pt;font-weight:600;color:#222;"><?= $nasc ?></div>
                    </div>
                    <div style="background:linear-gradient(135deg,#1b3a6b,#0f2347);padding:7px 12px;display:flex;align-items:center;justify-content:space-between;">
                        <span style="font-size:5.5pt;font-weight:800;letter-spacing:.5px;color:#fff;background:<?= $corSit ?>;padding:2px 7px;border-radius:10px;"><?= $situacao ?></span>
                        <?php if ($validade): ?>
                        <span style="font-size:5pt;color:rgba(255,255,255,.7);">Val. <strong style="color:#f0c845;"><?= e($validade) ?></strong></span>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="mt-3">
            ...
            <div class="card-body">
                <form method="POST">
                    ...
                    <div class="alert alert-info py-2 mb-3" style="font-size:.8rem;">
                        ...
                        <strong>Dados exibidos na carteira:</strong> foto, nome, profissão, formação acadêmica,
                        matrícula, CPF e data de nascimento são extraídos do cadastro de cada associado.
                    </div>
