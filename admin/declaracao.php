...
if (isset($_GET['imprimir']) && isset($_GET['user_id'])) {
    ...
    $user = db()->prepare("SELECT * FROM users WHERE id = ?")->execute([$uid]) ? null : null;
...
.decl-title {
    ...
    margin-bottom: 30px; color: #1b3a6b;
    border-bottom: 2px solid #1b3a6b; padding-bottom: 10px;
}
...
<body>
<div class="print-bar">
    <button onclick="window.print()"><i>🖨</i> Imprimir</button>
    <a href="<?= BASE_URL ?>/admin/declaracao.php?user_id=<?= $uid ?>">← Voltar</a>
...
<div class="page-wrap">
    <div class="a4">
        ...
        <div class="decl-local"><?= e($local) ?>, <?= date('d') ?> de <?= strftime('%B', time()) ?> de <?= date('Y') ?></div>
        <div class="decl-sign">
            <?php if ($assinatura): ?>
    ...
    foreach (['declaracao_fundo' => 'decl_bg', 'declaracao_assinatura' => 'decl_sign'] as $key => $field) {
        if (!empty($_FILES[$field]['name'])) {
            ...
            if ($up) {
                ...
                flash('warning', 'Falha no upload da imagem. Verifique formato/tamanho.');
            }
        }
    }
...
$texto     = getSetting('declaracao_texto', "Declaramos, para os devidos fins, que {nome}, portador(a) do CPF nº {cpf}, inscrito(a) com a Matrícula APEJESE nº {matricula}, encontra-se na situação de ASSOCIADO {situacao} junto à Associação dos Peritos Judiciais do Estado de Sergipe – APEJESE.\n\nPor ser expressão da verdade, firmamos a presente declaração.");
$assinante = getSetting('declaracao_assinante', '');
$cargo     = getSetting('declaracao_cargo', '');
$local     = getSetting('declaracao_local', 'Aracaju/SE');
$fundo     = getSetting('declaracao_fundo', '');
...
<div class="row g-4">
    ...
    <!-- Coluna lateral: variáveis + configurações -->
    <div class="col-lg-4">
        ...
        <div class="card admin-card" id="config">
            <div class="card-header"><i class="bi bi-gear-fill me-1"></i>Configurações da Declaração</div>
            <div class="card-body">
                <form method="POST" enctype="multipart/form-data">
                    <?= csrfField() ?>

                    <div class="mb-3">
                        <label class="form-label" style="font-size:.83rem;font-weight:700;color:var(--primary);">Texto da Declaração</label>
                        <textarea name="declaracao_texto" class="form-control" rows="8"
                                  style="font-size:.82rem;font-family:monospace;"><?= e($texto) ?></textarea>
                        <div class="form-text" style="font-size:.72rem;">Use as variáveis acima para inserir dados do associado.</div>
                    </div>
                    ...
                    <div class="mb-3">
                        <label class="form-label" style="font-size:.83rem;font-weight:700;color:var(--primary);">
                            Imagem de Fundo
                            <span style="font-weight:400;color:#999;">(marca d'água)</span>
                        </label>
                        <?php if ($fundo): ?>
                        <div class="mb-1">
                            <img src="<?= UPLOAD_URL . e($fundo) ?>" style="max-width:80px;max-height:50px;border-radius:4px;border:1px solid #ddd;">
                            <small class="text-muted ms-2">atual</small>
                        </div>
                        <?php endif; ?>
                        <input type="file" name="decl_bg" class="form-control form-control-sm" accept="image/*">
                        <div class="form-text" style="font-size:.72rem;">JPG, PNG, WEBP — será exibida com 8% de opacidade.</div>
                    </div>
